  ...
  try {
    if (req.user_id != null) {
      var bookmarks = await query(
        `SELECT post_id FROM bookmarks WHERE user_id=${req.user_id}`
      );
      var followings = await query(
        `SELECT following_id FROM follows WHERE user_id=${req.user_id}`
      );
    }
    ...
    for (let i = 0; i < posts.length; i++) {
      ...
      let users = await query(
        `SELECT first_name,last_name,profile_image FROM users WHERE user_id=${posts[i].user_id}`
      );
    ...
    const checkMorePosts = await query(
      `SELECT * FROM posts WHERE id>${index} order by id LIMIT 4`
    );
...
async function postDetails(req, res) {
  ...
  try {
    ...
    const author = await query(
      `SELECT first_name,last_name,profile_image,user_id FROM users WHERE user_id=${post.user_id}`
    );
    ...
    if (req.user_id != null) {
      ...
      var following = await query(
        `SELECT following_id FROM follows WHERE user_id=${req.user_id} AND following_id=${post.user_id}`
      );
      const bookmark = await query(
        `SELECT * FROM bookmarks WHERE user_id=${req.user_id} AND post_id=${post.id}`
      );
    ...
    let other_posts = await query(
      `SELECT * FROM posts WHERE user_id=${post.user_id} AND id!=${post_id} ORDER BY date ASC LIMIT 3`
    );
    ...
    if (other_posts.length > 0) {
      if (req.user_id != null)
        var bookmarks = await query(
          `SELECT * FROM bookmarks WHERE user_id=${req.user_id}`
        );

      post.setMorePosts(other_posts);
      for (let i = 0; i < post.getMorePosts().length; i++) {
        const tags = await query(
          `SELECT title FROM tags WHERE post_id=${post.getMorePosts()[i].id}`
        );
        ...
        if (req.user_id != null) {
          ...
          for (let j = 0; j < following.rows.length; j++) {
            if (
              post.getMorePosts()[i].user_id == following.rows[j].following_id
            ) {
...
async function createPost(req, res) {
  ...
  try {
    ...
    const insert =
      await query(`INSERT INTO posts(id,title,description,post_images,user_id,date,reading_time) 
    VALUES(${post_id},'${title}','${description}','${post_images}',${user_id},'${date.toLocaleDateString(
        "en-US"
      )}','${reading_time}') RETURNING id`);

    // for (let i = 0; i < tags.length; i++)
    await query(
      `INSERT INTO tags(post_id,title) VALUES(${post_id}, '${tags}')`
    );
  ...
  try {
    if (like > 0) {
      const duplicate = await query(
        `SELECT COUNT(*) FROM likes WHERE user_id=${user_id} AND post_id=${post_id}`
      );
...
async function getUserPosts(req, res) {
  ...
  try {
    ...
    if (req.main_user_id) {
      var bookmarks = await query(
        `SELECT post_id FROM bookmarks WHERE user_id=${req.main_user_id}`
      );
    }
    ...
    for (let i = 0; i < user_posts.length; i++) {
      ...
      let users = await query(
        `SELECT first_name,last_name,profile_image FROM users WHERE user_id=${user_posts[i].user_id}`
      );

      let tags = await query(
        `SELECT title FROM tags WHERE post_id=${user_posts[i].id}`
      );

      let is_liked = await query(
        `SELECT COUNT(*) FROM likes WHERE user_id=${req.main_user_id} AND post_id=${user_posts[i].id}`
      );
      if (is_liked.rows[0].count > 0) user_posts[i].setIsLiked(true);
      else user_posts[i].setIsLiked(false);

      let comment_number = await query(
        `SELECT COUNT(*) FROM comments WHERE post_id=${user_posts[i].id} `
      );
